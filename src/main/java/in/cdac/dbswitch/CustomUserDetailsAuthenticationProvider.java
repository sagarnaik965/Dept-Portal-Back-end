...
public class CustomUserDetailsAuthenticationProvider 
    ...
    protected UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken authentication)
    	...
    	logger.info("====>"+authentication.getClass().getTypeName());
        CustomAuthenticationToken auth = (CustomAuthenticationToken) authentication;
        UserDetails loadedUser;
     //   String en=passwordEncoder.encode("welcome@123");

        try {
            loadedUser = this.userDetailsService.loadUserByUsernameAndTenantname(auth.getPrincipal().toString(),auth.getTenant());
